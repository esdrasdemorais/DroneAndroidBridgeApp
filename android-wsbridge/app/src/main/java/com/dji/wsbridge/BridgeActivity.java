...
public class BridgeActivity extends Activity {
    ...
    private static final Observable HEART_BEAT = Observable.timer(
        2, TimeUnit.SECONDS).repeat().observeOn(AndroidSchedulers.mainThread()
    );
    ...
    private TextView mBatteryRemainingTextView;
    ...
    protected void onNewIntent(Intent intent) {
        switch (intent.getAction()) {
            case UsbManager.ACTION_USB_ACCESSORY_ATTACHED:
                BridgeApplication.getInstance().getBus().post(
                    new USBConnectionManager.USBConnectionEvent(true)
                );
                break;
        }
    ...
    private void setupViews() {
        ...
        mBatteryRemainingTextView = (TextView) findViewById(R.id.batteryRemaining);
            ...
            public void run() {
                ...
                if (isUSBConnected.get()) {
                    if (isRCConnected.get()) {
                        mRCIconView.setColorFilter(getResources().getColor(
                            android.R.color.holo_green_light), PorterDuff.Mode.MULTIPLY);
                        DJILogger.logConnectionChange(DJILogger.CONNECTION_RC,
                            DJILogger.CONNECTION_GOOD);
                    } else {
                        mRCIconView.setColorFilter(getResources().getColor(
                            android.R.color.holo_purple), android.graphics.PorterDuff.Mode.MULTIPLY);
                    ...
                    if (isRCConnected.get()) {
                        ...
                        mRCIconView.setColorFilter(getResources().getColor(
                            android.R.color.holo_red_light), PorterDuff.Mode.MULTIPLY);
    ...
    public void onRCConnectionEvent(USBConnectionManager.RCConnectionEvent event) {
        if (isRCConnected.compareAndSet(!event.isConnected(), event.isConnected())) {
            ...
            // In some this event comes after runners have been already started so we need to
            // restart runner to make bridge work
            refreshRunners();
        }
    }
    //endregion -----------------------------------------------------------------------------------------------------
    ...
    public void onWSTrafficEvent(WSConnectionManager.WSTrafficEvent event) {
        ...
        if (event.isSlowConnection()) {
            if (isWSTrafficSlow.compareAndSet(false, true)) {
                shouldRefresh = true;
                showToast("Bad Network Connection: " + event.getMessage() +
                    "!", isWiFiConnected.get(), event.getMessage());
            }
        } else {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                Toast.makeText(getApplicationContext(),
                    finalMessage, Toast.LENGTH_SHORT
                ).show();
    ...
    public void showBatteryRemainingCharge() {
        DJISDKManager.getInstance().getProduct().getBattery().setStateCallback(
    ...
    public void enableBridgeModeWithBridgeAppIP() {
        try {
            String ipAddress = getString(R.id.iptextView);
            // enabled bridge app at this ip address
